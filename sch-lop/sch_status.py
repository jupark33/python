...
import json
from util.StringUtil import strCommaToInt, change_hour_minute, timestamp_to_time
...
#         print("[%s] %s task id[%s] volume[%s]" % (str_now, type, job_id, volume)) #ok

    # 스케줄러생성
    def scheduler(self, type, job_id):
        str_now = getNow()
        print("[%s] Scheduler Start" % str_now, type)
        self.sched.add_job(self.task, type, day_of_week='mon-fri', hour='0-23', second='*/10', id=job_id, args=(type, job_id))
